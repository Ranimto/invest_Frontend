...
const Stock = () => {
 ...
 const response2 = await axios.get(`http://localhost:8023/stockData/fetchData/${symbol}/IBM/MEAN`);


 console.log ("SYMBOL",symbol);
 const payload = response2.payload.RETURNS_CALCULATIONS.STDDEV.RUNNING_STDDEV[symbol];
 const stockEntries = [];  // Array to store extracted data

 for (const [date, value] of Object.entries(payload)) {
  stockEntries.push({ date, value });
}
  setAnalyticData(extractedData);
